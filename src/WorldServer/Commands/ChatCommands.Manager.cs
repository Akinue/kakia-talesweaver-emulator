...
namespace Kakia.TW.World.Commands
{
	...
	public partial class ChatCommands : CommandManager<ChatCommand, ChatCommandFunc>
	{
		...
		/// <returns></returns>
		public bool TryExecute(Player character, string message)
		{
			var prefix = WorldServer.Instance.Conf.Commands?.SelfPrefix ?? DefaultPrefix;
			...
			if (command == null)
			{
				SendServerMessage(character, Localization.Get("Unknown command."));
				Log.Warning(Localization.Get("Unknown command: {0}, message: {1}."), commandName, message);
				return true;
			}

			// Check levels
			var levels = WorldServer.Instance.Conf.Commands?.GetLevels(command.Name);
			if (levels == null)
			{
				levels = WorldServer.Instance.Conf.Commands?.GetLevels("default");
				if (levels == null)
				{
					// No command config - allow all commands for now (dev mode)
					Log.Debug("Commands: No authority levels configured, allowing all commands.");
				}
			}

			var authority = character.Connection.Account?.Authority ?? 0;

			if (levels != null && ((!isDoublePrefix && authority < levels.Self) || (isDoublePrefix && authority < levels.Target)))
			{
				SendServerMessage(character, Localization.Get("You're not authorized to use this command."));
				return true;
			}
			...
			if (isDoublePrefix)
			{
				if (args.Count < 2)
				{
					SendServerMessage(character, Localization.Get("No target specified."));
					return true;
				}

				var targetName = args.Get(1);
				if (!WorldServer.Instance.World.TryGetPlayerByName(targetName, out target!))
				{
					SendServerMessage(character, Localization.Get("Target character not found."));
					return true;
				}
			...
			switch (result)
			{
				...
				case CommandResult.Fail:
					SendServerMessage(character, Localization.Get("Failed to execute command."));
					break;

				case CommandResult.InvalidArgument:
					SendServerMessage(character, string.Format(Localization.Get("Invalid argument, usage: {0}{1} {2}"), prefix, command.Name, command.Usage));
					break;
		...
		/// <summary>
		/// Sends a server message to the player using the chat system.
		/// </summary>
		private static void SendServerMessage(Player player, string message)
		{
