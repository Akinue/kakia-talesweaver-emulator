	...
	public class Heartbeat
	{
		...
		private void Loop()
		{
			...
			while (true)
			{
				...
				UpdateAverageUpdates(elapsed);
				UpdateAverageUpdateTime(updateTimer.Elapsed);
		...
		private void UpdateAverageUpdates(TimeSpan elapsed)
		{
			...
			{
				UpdatesPerSecond = _updatesCount;
		...
		private void UpdateAverageUpdateTime(TimeSpan elapsed)
		{
			...
			{
				AverageUpdateTime = TimeSpan.FromTicks(_averageUpdateTickTotal.Ticks / _averateUpdateTickCount);
		...
		private void OnTick(TimeSpan elapsed)
		{
			try
			{
				UpdateUpdateables(elapsed);
				HeartbeatTick?.Invoke(elapsed);
			}
			catch (Exception ex)
			{
