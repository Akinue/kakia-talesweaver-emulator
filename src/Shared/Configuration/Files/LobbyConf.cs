...
{
	/// <summary>
	/// Represents lobby.conf.
	/// </summary>
	public class LobbyConf : ConfFile
	{
		...
		public void Load(string filePath)
		{
			...
			this.BindPort = this.GetInt("char_bind_port", 20001);
			this.ServerIp = this.GetString("char_server_ip", "127.0.0.1");
			this.Name = this.GetString("char_server_name", "Kakia");
		...
		private void ReadStartLocation(string name, string defaultValue)
		{
			...
			else if (!ushort.TryParse(split[1], out var x) || !ushort.TryParse(split[2], out var y))
			{
